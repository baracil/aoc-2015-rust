...
impl Default for Reader {
    fn default() -> Self {
        Reader{ previous:Previous::None, result:"".to_string()}
    }
}
        ...
        match &self.previous {
            ...
            Some(previous,count) => {
                if *previous == c {
                    ...
                    self.result.push_str(format!("{}{}",count,previous).as_str());
...
pub fn read(line:&str) -> String {
    let mut reader= line.chars().fold(Reader::default(), | mut acc,c| {acc.push_char(c);acc} );
